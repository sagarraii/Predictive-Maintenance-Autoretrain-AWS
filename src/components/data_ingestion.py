...
class DataIngestion:
    ...
    def download_data(self):
        try:
            ...
            os.makedirs(self.config.data_download_dir,exist_ok=True )
            api = KaggleApi()
            api.authenticate()

            api.dataset_download_files(self.config.dataset_name, path=self.config.data_download_dir, unzip=True)
